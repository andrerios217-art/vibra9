# -*- coding: utf-8 -*-
import uuid, hashlib, secrets, re
...
def register(payload: RegisterRequest):
    if not payload.privacy_policy_accepted or not payload.terms_accepted:
        raise HTTPException(status_code=400, detail="Aceite a política de privacidade e os termos de uso.")

    if not re.search(r"[A-Za-z]", payload.password) or not re.search(r"\d", payload.password):
        raise HTTPException(status_code=400, detail="A senha deve conter pelo menos uma letra e um número.")
    ...
    conn.execute("""INSERT INTO users
        (id,name,email,password_hash,privacy_policy_accepted,privacy_policy_accepted_at,privacy_policy_version,
         terms_accepted,terms_accepted_at,terms_version,email_verified,subscription_status,trial_start,trial_end,created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (uid, payload.name.strip(), email, hash_password(payload.password),
         1, now_s, "1.0", 1, now_s, "1.0", 0, "trial", now_s, trial_end, now_s))
    conn.execute("""INSERT INTO consent_log
        (id, user_id, privacy_policy_version, terms_version, accepted_at)
        VALUES (?,?,?,?,?)""",
        (str(uuid.uuid4()), uid, "1.0", "1.0", now_s))
    conn.commit()
    conn.close()
...
def refresh_token(payload: RefreshRequest):
    ...
    conn = get_connection()
    stored = conn.execute(
        "SELECT * FROM refresh_tokens WHERE token_hash=? AND user_id=? AND revoked=0 AND expires_at > ?",
        (th, user_id, now_iso)).fetchone()
    if not stored:
        conn.close()
        raise HTTPException(status_code=401, detail="Refresh token inválido, expirado ou revogado.")
...
def verify_email(payload: VerifyEmailRequest, user: Dict[str, Any] = Depends(get_current_user)):
    if user.get("email_verified", 0):
        return {"verified": True, "message": "E-mail já verificado."}
    conn = get_connection()
    row = conn.execute(
    ...
    if not row:
        conn.close()
        raise HTTPException(status_code=400, detail="Nenhum código ativo. Solicite um novo.")
    code_row = dict(row)
...
def forgot_password(payload: ForgotPasswordRequest):
    ...
    conn.close()
    generic_msg = "Se o e-mail existir, você receberá instruções de redefinição."
    if not user:
